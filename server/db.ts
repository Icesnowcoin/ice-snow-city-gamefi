...
export async function applyPlayerNftHoldingDelta(delta: { userId: number; walletAddress: string; chainId: number; nftContract: string; tokenId: string; amount: string; lastSyncedBlock: string }) {
  ...
  const nextAmount = BigInt(existing[0]?.amount ?? "0") + BigInt(delta.amount);
  if (nextAmount < BigInt(0)) throw new Error("持仓增量会导致负数余额，拒绝写入");
  if (existing[0]) {
    return db.update(playerNftHoldings).set({ amount: nextAmount.toString(), lastSyncedBlock: delta.lastSyncedBlock }).where(eq(playerNftHoldings.id, existing[0].id));
  }
  return db.insert(playerNftHoldings).values({ ...delta, walletAddress: delta.walletAddress.toLowerCase(), nftContract: delta.nftContract.toLowerCase() });
}
