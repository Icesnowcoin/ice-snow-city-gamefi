...
import { gasOptimizationService } from "../game-logic/gasOptimization";
...
export const npcRouter = router({
  ...
  /**
   * Get NPC current location and activity
   */
  getCurrentStatus: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const availability = NPCScheduleService.getAvailabilityStatus(
        input.npcId,
        state.gameTime,
        state.npcSchedules
      );
      const timeUntilAvailable = NPCScheduleService.getTimeUntilAvailable(
        input.npcId,
        state.gameTime,
        state.npcSchedules
      );
  ...
  /**
   * Get NPC 24-hour schedule
   */
  getSchedule24Hours: protectedProcedure
    .input(z.object({ npcId: z.string() }))
    .query(async ({ ctx, input }) => {
      const userId = ctx.user!.id;
      const state = await getGameState(String(userId), userId);
      ...
      return {
        npcId: input.npcId,
        schedule,
        currentHour: state.gameTime.hour,
      };
    }),

  /**
   * Get all available NPCs
   * Data source priority: npcRelationships > npcSchedules > fallback
   */
  getAvailableNPCs: protectedProcedure.query(async ({ ctx }) => {
    const userId = ctx.user!.id;
    const state = await getGameState(String(userId), userId);
    
    let allNPCIds: string[] = [];
    
    if (state.npcRelationships && state.npcRelationships.length > 0) {
      allNPCIds = state.npcRelationships.map((rel) => rel.npcId);
    } else if (state.npcSchedules && Object.keys(state.npcSchedules).length > 0) {
      allNPCIds = Object.keys(state.npcSchedules);
    } else {
      allNPCIds = ["npc_001", "npc_002", "npc_003", "npc_004", "npc_005"];
    }
    
    const availableNPCs = NPCScheduleService.getAvailableNPCs(state, allNPCIds);
...
export const economyRouter = router({
  ...
  getMarketPrice: protectedProcedure
    .input(z.object({ itemId: z.string(), demandLevel: z.enum(["low", "medium", "high"]).optional() }))
    .query(async ({ ctx, input }) => {
      const userId = ctx.user!.id;
      const state = await getGameState(String(userId), userId);
      const price = EconomyCycleService.calculateMarketPrice(
        input.itemId,
        state.gameTime,
        input.demandLevel || "medium"
      );
      return { itemId: input.itemId, price, currency: "ISC" };
    }),

  /**
   * 获取所有市场价格
   */
  getAllMarketPrices: protectedProcedure.query(async ({ ctx }) => {
    const userId = ctx.user!.id;
    const state = await getGameState(String(userId), userId);
    const items = ["wheat", "corn", "rice", "tomato", "carrot", "apple", "milk", "egg", "meat", "fish"];
    const prices = items.map(itemId => ({
      itemId,
      price: EconomyCycleService.calculateMarketPrice(itemId, state.gameTime, "medium"),
    }));
    return { prices, season: state.gameTime.season, gameTime: state.gameTime };
  }),

  /**
   * 获取季节信息
   */
  getSeasonInfo: protectedProcedure.query(async ({ ctx }) => {
    const userId = ctx.user!.id;
    const state = await getGameState(String(userId), userId);
    const modifier = EconomyCycleService.getSeasonalModifier(state.gameTime);
    return {
      season: state.gameTime.season,
      modifier,
...
import { NPCScheduleService } from "../game-logic/npcSchedule";
import { AchievementService } from "../game-logic/achievements";
import { LeaderboardService } from "../game-logic/leaderboard";
import { EconomyCycleService } from "../game-logic/economyCycle";
import { RandomEventService } from "../game-logic/randomEvents";
...
export const gameCoreRouter = router({
  ...
  walletTransfer: protectedProcedure
    .input(z.object({
      ...
      currency: z.enum(["money", "isc"]).default("money"),
    }))
    .mutation(async ({ ctx, input }) => {
      const userId = ctx.user!.id;
      const playerId = String(userId);
      const state = await getGameState(playerId, userId);
      const balance = input.currency === "money" ? state.wallet.money : state.wallet.isc;
      if (balance < input.amount) {
        throw new TRPCError({ code: "BAD_REQUEST", message: "Insufficient balance" });
      }
      if (input.toPlayerId === playerId) {
        throw new TRPCError({ code: "BAD_REQUEST", message: "Cannot transfer to yourself" });
      }
      const action = EconomyService.transfer(state, input.toPlayerId, input.amount, input.currency);
      return dispatchAction(playerId, userId, action);
    }),
...
export const achievementRouter = router({
  getList: publicProcedure.query(async () => {
    ...
    const { ACHIEVEMENTS } = require("../game-logic/achievements");
    return Object.values(ACHIEVEMENTS).map((achievement: any) => ({
      id: achievement.id,
  ...
  getUnlocked: protectedProcedure.query(async ({ ctx }) => {
    ...
    const state = gameStatesCache.get(String(ctx.user.id));
    if (!state) {
      return [];
    }
    ...
    for (const achievement of Object.values(ACHIEVEMENTS) as any[]) {
      if (achievement.unlockCondition(state, state.progress)) {
  ...
  checkProgress: protectedProcedure.query(async ({ ctx }) => {
    // Get player's game state
    const state = gameStatesCache.get(String(ctx.user.id));
    if (!state) {
...
export const leaderboardRouter = router({
  getWealthRanking: publicProcedure.query(async () => {
    // Get all game states from cache
    const entries: any[] = [];
    let rank = 1;

    // Sort by wealth score
    const sortedStates = Array.from(gameStatesCache.entries())
      .map(([playerId, state]) => ({
        playerId,
        state,
        wealthScore: LeaderboardService.calculateWealthScore(state),
      }))
      .sort((a, b) => b.wealthScore - a.wealthScore);

    for (const { playerId, state, wealthScore } of sortedStates.slice(0, 100)) {
      entries.push({
        rank: rank++,
        playerId,
  ...
  getLevelRanking: publicProcedure.query(async () => {
    // Get all game states from cache
    const entries: any[] = [];
    let rank = 1;
    ...
    for (const { playerId, state, level } of sortedStates.slice(0, 100)) {
      entries.push({
        rank: rank++,
        playerId,
        playerName: state.player.name,
  ...
  getExperienceRanking: publicProcedure.query(async () => {
    // Get all game states from cache
    const entries: any[] = [];
    let rank = 1;
    ...
    for (const { playerId, state, totalExperience } of sortedStates.slice(0, 100)) {
      entries.push({
        rank: rank++,
        playerId,
        playerName: state.player.name,
        ...
        metadata: {
          level: state.player.level,
          currentExperience: state.player.experience,
        },
      });
    }
  ...
  getSocialRanking: publicProcedure.query(async () => {
    // Get all game states from cache
    const entries: any[] = [];
    let rank = 1;

    // Sort by NPCs friended
    const sortedStates = Array.from(gameStatesCache.entries())
      .map(([playerId, state]) => ({
        playerId,
        state,
      ...
      .sort((a, b) => b.npcsFriended - a.npcsFriended);

    for (const { playerId, state, npcsFriended } of sortedStates.slice(0, 100)) {
      entries.push({
        rank: rank++,
        playerId,
        playerName: state.player.name,
  ...
  getPlayerRank: protectedProcedure
    .input(z.object({ type: z.enum(["wealth", "level", "experience", "social"]) }))
    .query(async ({ ctx, input }) => {
      const playerState = gameStatesCache.get(String(ctx.user.id));
      if (!playerState) {
        return null;
      }
      ...
      switch (input.type) {
        case "wealth":
          playerScore = LeaderboardService.calculateWealthScore(playerState);
          allScores = Array.from(gameStatesCache.values()).map((state) =>
            LeaderboardService.calculateWealthScore(state)
          );
          break;
        case "level":
          playerScore = playerState.player.level;
          allScores = Array.from(gameStatesCache.values()).map((state) => state.player.level);
          break;
        case "experience":
          playerScore = playerState.player.totalExperience;
          allScores = Array.from(gameStatesCache.values()).map(
            (state) => state.player.totalExperience
          );
          break;
        case "social":
          playerScore = playerState.progress.npcsFriended;
          allScores = Array.from(gameStatesCache.values()).map(
            (state) => state.progress.npcsFriended
          );
          break;
      }

      const rank = allScores.filter((score) => score > playerScore).length + 1;

      return {
        rank,
        value: playerScore,
        totalPlayers: gameStatesCache.size,
        percentile: Math.round(((gameStatesCache.size - rank + 1) / gameStatesCache.size) * 100),
      };
    }),
});
