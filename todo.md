...
## Phase 13 实现细节 (需要完善)
- [x] 实现真实玩家仪表板：用 tRPC 接入资产、ISC 余额、排名与活动数据
- [x] 添加 loading/error/empty states 到仪表板和钱包页面
- [x] 实现真实钱包流程：充值/提现/转账表单校验、后端调用
- [x] 修复 Gas 费用显示逻辑（玩家承担提现 Gas，系统承担充值 Gas）
- [x] 实现真实交易记录与统计：从后端查询交易历史、筛选/分页
- [x] 实现玩家资料编辑功能和真实数据加载
- [x] 单独实现银行系统界面：存入、取出、利息/APY 计算
