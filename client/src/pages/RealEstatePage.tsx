...
import RealEstatePlacementManager from "@/components/realEstate/RealEstatePlacementManager";
...
export default function RealEstatePage() {
  ...
  const { data: walletBalance, isLoading: isBalanceLoading } = trpc.wallet.getBalance.useQuery(undefined, {
    retry: false,
  ...
  const placementLandPlots = useMemo(() => ownedProperties.map((property) => ({
    ...
    width: property.area >= 500 ? 16 : 12,
    height: property.area >= 500 ? 12 : 8,
  })), [ownedProperties]);
  const iscBalance = walletBalance?.iscBalance ? Number(walletBalance.iscBalance) : undefined;
  ...
  return (
    <div className="space-y-6">
      ...
      <RealEstatePlacementManager
        landPlots={placementLandPlots}
        iscBalance={iscBalance}
