...
export default function WalletPage() {
  ...
  const transferMutation = trpc.game.core.walletTransfer.useMutation({
    ...
    onError: (err) => {
      toast.error(lang === "zh" ? `转账失败: ${err.message}` : `Transfer failed: ${err.message}`);
    },
  });
  ...
  const handleTransfer = async () => {
    ...
    if (!transferTo.trim()) {
      toast.error(lang === "zh" ? "请输入收款人 ID" : "Please enter recipient ID");
      return;
    }
    if (!transferAmount || isNaN(amount) || amount < 100) {
      toast.error(lang === "zh" ? "最低转账 100 ISC" : "Minimum transfer is 100 ISC");
      return;
    }
    ...
    try {
      await transferMutation.mutateAsync({ toPlayerId: transferTo.trim(), amount, currency: "money" });
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Dialog>
          ...
          <DialogContent>
            ...
            <div className="space-y-4">
              <div>
                <Label htmlFor="transfer-to">{lang === "zh" ? "收款人 ID" : "Recipient ID"}</Label>
                <Input
                  id="transfer-to"
                  placeholder={lang === "zh" ? "输入玩家 ID" : "Enter player ID"}
                  value={transferTo}
              ...
              <p className="text-xs text-muted-foreground">
                {lang === "zh" ? "转账将从您的钱包余额中扣除，无手续费。" : "Transfer deducted from wallet balance, no fees."}
              </p>
      ...
      <Tabs defaultValue="transactions" className="w-full">
        <TabsList className="grid w-full grid-cols-3">
          ...
          <TabsTrigger value="share-stats">{lang === "zh" ? "分享统计" : "Share Stats"}</TabsTrigger>
        </TabsList>
                  ...
                  <div key={tx.id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-accent transition-colors">
                    ...
                    <div className="flex items-center gap-3 flex-shrink-0">
                      <div className="text-right">
                        <p className={`font-medium ${tx.type === "deposit" ? "text-green-600" : tx.type === "transfer" ? "text-blue-600" : "text-red-600"}`}>
        ...
        {/* Share Statistics Tab */}
        <TabsContent value="share-stats">
      ...
      {/* Blockchain Sync Overlay */}
      {syncStatus !== "idle" && (
        <BlockchainSyncOverlay
          isVisible={true}
          status={syncStatus as "syncing" | "success" | "error"}
          message={syncMessage}
