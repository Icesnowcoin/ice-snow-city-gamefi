...
describe("assetManifest", () => {
  ...
  it("keeps baseline preview URLs separate from undelivered real assets", () => {
    expect(CORE_ASSET_MANIFEST).toHaveLength(8);
    for (const entry of CORE_ASSET_MANIFEST) {
      expect(entry.glbUrl).toBeNull();
      expect(entry.baselineGlbUrl).toMatch(/^\/manus-storage\/.*-baseline_.*\.glb$/);
      expect(entry.status).toBe("pending-import");
