...
export async function runSnowLayerPerfBenchmark(
  ...
  const durationMs = Math.max(3000, options.durationMs ?? 10000);
  const quality = options.quality ?? "medium";
  const meshCount = Math.max(1, Math.floor(options.meshCount ?? 24));
  ...
  for (const mode of ["baseline", "snow"] as const) {
    ...
    samples.push(await collectSample(engine, scene, mode, quality, durationMs, options.onProgress));
  }
...
async function waitForStableFrames(engine: Engine, scene: Scene, frames: number): Promise<void> {
