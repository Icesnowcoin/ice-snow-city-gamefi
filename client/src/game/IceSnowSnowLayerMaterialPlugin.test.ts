...
describe("IceSnowSnowLayerMaterialPlugin", () => {
  ...
  it("keeps quality-specific shader branches for low, medium and high tiers", () => {
    const { engine, scene, material, plugin } = createPlugin({ quality: "medium" });
    const shader = plugin.getCustomCode("fragment")?.CUSTOM_FRAGMENT_DEFINITIONS ?? "";

    expect(shader).toContain("#if defined(ISC_SNOW_HIGH)");
    expect(shader).toContain("#elif defined(ISC_SNOW_MEDIUM)");
    expect(shader).toContain("return step(iscSnowSlopeThreshold, n.y) * 0.82");

    plugin.setQuality("low");
    const lowDefines = {} as Record<string, boolean>;
    plugin.prepareDefines(lowDefines as never, scene, material as never);
    expect(lowDefines.ISC_SNOW_LAYER).toBe(true);
    expect(lowDefines.ISC_SNOW_HIGH).toBe(false);
    expect(lowDefines.ISC_SNOW_MEDIUM).toBe(false);
