...
export const GameTasks: React.FC = () => {
  ...
  const { data: tasks, isLoading: tasksLoading, refetch: refetchTasks } = 
    trpc.gameCore.task.getAvailableTasks.useQuery(undefined, { staleTime: 30000 });

  // 接受任务 mutation
  const acceptMutation = trpc.gameCore.task.acceptTask.useMutation({
    onSuccess: () => {
      refetchTasks();
      console.log("任务已接受");
    },
  ...
  // 完成任务 mutation
  const completeMutation = trpc.gameCore.task.completeTask.useMutation({
    onSuccess: () => {
      refetchTasks();
      console.log("任务已完成");
    },
