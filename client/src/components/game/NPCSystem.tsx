...
import { Search, Heart, MessageCircle, Gift, Star, Loader2, AlertCircle, CheckCircle2, Clock } from 'lucide-react';
import { trpc } from '@/lib/trpc';
import NPCSchedulePanel from '@/components/NPCSchedulePanel';
import type { NPCSchedulePanelData } from '@/components/NPCSchedulePanel';
import {
  useNPCList,
  useNPCDetail,
  useNPCCurrentStatus,
  useNPCSchedule24Hours,
  useAvailableNPCs,
  usePrefetchNPCAll,
} from '@/hooks/useNPCDataWithCache';
import {
  NPCListSkeleton,
  NPCDetailSkeleton,
  NPCScheduleSkeleton,
...
import AdvancedErrorBoundary, { ErrorBoundarySection } from '@/components/AdvancedErrorBoundary';
...
export function NPCSystem() {
  ...
  const { lastUpdateTime, isRefreshing, isError, errorMessage, handleRefresh } =
    useRefreshControl();
  const { refetch: refetchNPCList } = useNPCList('main_square');
  const { refetch: refetchAvailableNPCs } = useAvailableNPCs();

  const handleManualRefresh = async () => {
    await handleRefresh(async () => {
      await Promise.all([
        refetchNPCList(),
        refetchAvailableNPCs(),
        selectedNPC?.id ? refetchNPCDetail() : Promise.resolve(),
      ]);
    });
  };

  // Fetch NPC list from backend with optimized cache
  const { data: npcList, isLoading: npcListLoading } = useNPCList('main_square');

  // Fetch NPC detail with optimized cache
  const { data: npcDetail, refetch: refetchNPCDetail } = useNPCDetail(selectedNPC?.id);

  // Fetch available NPCs with optimized cache
  const { data: availableNPCsData, isLoading: availableNPCsLoading } = useAvailableNPCs();

  // Fetch NPC schedule for selected NPC with optimized cache
  const { data: npcScheduleData, isLoading: scheduleLoading } = useNPCSchedule24Hours(
    selectedNPC?.id,
    showSchedulePanel // Only enable when schedule panel is open
  );

  // Fetch NPC current status with optimized cache
  const { data: npcStatusData, isLoading: statusLoading } = useNPCCurrentStatus(
    selectedNPC?.id,
    showSchedulePanel // Only enable when schedule panel is open
  );

  // Prefetch hook for warming up cache
  const prefetchNPCAll = usePrefetchNPCAll();
  ...
  // Prefetch NPC data when user hovers over or selects an NPC
  const handleNPCHover = (npcId: string) => {
    // Prefetch detail and schedule data when user hovers
    prefetchNPCAll(npcId).catch(() => {
      // Silently fail - data will load when needed
    });
  };

  // Prefetch NPC data when schedule panel opens
  useEffect(() => {
    if (showSchedulePanel && selectedNPC?.id) {
      // Prefetch schedule and status data when panel opens
      prefetchNPCAll(selectedNPC.id).catch(() => {
        // Silently fail - data will load when needed
      });
    }
  }, [showSchedulePanel, selectedNPC?.id, prefetchNPCAll]);
  ...
  return (
    <AdvancedErrorBoundary level="section" onError={(error) => {
      console.error('NPC System Error:', error);
      setFeedback({ type: 'error', message: 'NPC 系统加载失败，请重试' });
    }}>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <RefreshControl
              ...
              isLoading={isRefreshing || npcListLoading}
              isError={isError}
                ...
                {npcListLoading ? (
                  ...
                  filteredNPCs.map((npc) => (
                    <div
                      ...
                      onMouseEnter={() => handleNPCHover(npc.id)}
        ...
        {/* NPC Schedule Panel */}
        {showSchedulePanel && selectedNPC && npcStatusData && (
          <div className="mt-6">
            {npcScheduleData?.schedule ? (
                ...
                npcs={[
                  {
                    ...
                    status: npcStatusData,
                    schedule24Hours: npcScheduleData.schedule,
                  } as NPCSchedulePanelData,
                ]}
                availableNPCIds={availableNPCsData?.availableNPCs || []}
                isLoading={statusLoading || scheduleLoading}
                onNPCSelect={(npcId) => {
              ...
              <div className="flex items-center justify-center py-8">
                ...
                <span className="text-gray-600">加载日程中...</span>
              </div>
            ) : (
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 flex items-start">
                <AlertCircle className="w-5 h-5 text-yellow-600 mr-3 flex-shrink-0 mt-0.5" />
                <p className="text-yellow-800">无法加载 NPC 日程，请稍后重试</p>
              </div>
            )}
          </div>
        )}
