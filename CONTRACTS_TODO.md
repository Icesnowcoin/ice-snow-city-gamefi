...
| 合约名称 | 文件名 | 完成度 | 说明 |
| :--- | :--- | :--- | :--- |
| ISCTradingCenter | ISC_Treasury_SmartContract.sol | ✅ 设计记录 | 商品交易；费用比例以具体部署版本和审计结论为准 |
...
**职责**：
- 管理进入国库钱包的游戏业务资金；来源、比例和用途按具体场景规则记录，不将旧 10% 佣金泛化
- 支持支出提案与投票
- 实现 DAO 治理机制
- 财务透明公示
...
**职责**：
- 自动计算各类税费
- 通过玩家主动授权的 ERC-20 `transferFrom` 结算已确认税费；不得从 ISC 代币合约侧强制扣款
- 支持税收豁免与减免
...
- **房产税**：按房产价值的 0.5% 年税
- **企业所得税**：按利润的 10% 年税
- 交易税：仅在具体市场合约明确定义并通过审计后适用；不得默认套用 10%
...
> ISC 约束：ISC 是已放弃所有权、代理和控制权的普通 ERC-20。NFT 铸造采用 1%销毁、69%国库、30%营销；社交消费采用 60%国库、40%营销。所有消耗由玩家主动授权并承担 Gas，具体执行以已部署合约和审计结果为准。
